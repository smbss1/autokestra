...
- Add a new official plugin `core/script` with action `run` to execute JS/TS from local path, inline content, or Git repository source.
- Support private Git repositories in v1 with token-based auth and SSH-based auth.
...
### New Capabilities
- `script-runner-plugin`: Run JS/TS scripts and package-based projects from local, inline, and Git sources with optional install/lifecycle handling.
...
- Runtime impact: plugin process execution, git checkout handling, optional package manager invocation, lifecycle orchestration.
- Security impact: execution of user-specified commands under explicit user control; requires clear logging, timeout handling, and explicit secret-based auth injection for private Git.
